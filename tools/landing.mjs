...
/** The pair every theme contributes: a cover and a body slide. */
const SWITCHER_TYPES = ["title", "bullets"];
...
async function emit(src, file, width) {
  ...
  await sharp(src)
    ...
    .toFile(file);
  return { w: width, h: height, bytes: (await stat(file)).size };
}

const manifest = { deck: path.relative(ROOT, DECK), themes: [], showcase: null };
let total = 0;
...
for (const name of themes) {
  ...
  manifest.themes.push({
    ...
    // The page dims its chrome around a dark specimen; without this it has to
    // guess from the image, which it cannot do before the image loads.
    dark: isDark(theme),
...
/**
 * Whether a theme's page is dark. Read off the rendered ground rather than a
 * declared flag: a plate theme's background is an image, and its palette token
 * is not what ends up on the page.
 */
function isDark(theme) {
  const bg = theme.surfaces?.background ?? theme.palette?.background ?? "#FFFFFF";
  const hexv = String(bg).replace("#", "").slice(0, 6);
  if (hexv.length < 6) return false;
  const [r, g, b] = [0, 2, 4].map((i) => parseInt(hexv.slice(i, i + 2), 16) / 255);
