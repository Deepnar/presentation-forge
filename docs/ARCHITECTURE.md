...
- `FORGE_HOSTED=1` (hosted): `autoProvider()` returns the gateway's `qwen3.6` when `FORGE_AUTO_API_KEY` is set (or the legacy `FORGE_TCET_API_KEY`, still read) and `null` otherwise — no local Ollama fallback. `resolveRole()` and `modelChoices()` hide local models, `chat()` for `role=author` surfaces a clear "Hosted auto not configured — switch to Cloud" error instead of "Ollama unreachable". Internal roles (`research`/`critic`/`utility`) fall through Auto → Cloud when hosted.
